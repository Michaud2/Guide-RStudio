...
Pour suivre les exemples qui seront présentés dans ce guide, vous pouvez importer la base de données en format **.csv (français)** qui est hébergée sur le site Dimension via le lien suivant en faisant un copier coller (SANS GUILLEMETS) dans la console lorsque ce sera demandé, ou en l'ajoutant directement comme valeur de l'argument data : <https://dimension.usherbrooke.ca/donnees/base2015CES.csv>

```{r base_importer, eval = FALSE}

fichier = tcltk::tk_choose.files()
fichier = file.choose()
fichier = choose.files()

base = importer(data = fichier)
# OU
base = importer(data = "https://dimension.usherbrooke.ca/donnees/base2015CES.csv")
```
