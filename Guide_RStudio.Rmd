...
Pour suivre les exemples qui seront présentés dans ce guide, vous pouvez importer la base de données en format *.csv français* qui est hébergée sur le lien suivant en faisant un copier coller (SANS GUILLEMETS) dans la console lorsque ce sera demandé, ou en l'ajoutant directement comme valeur de l'argument data : <https://dimension.usherbrooke.ca/donnees/base2015CES.csv>
...
freq_sexe = SommaireQuali(base$sex.r, na.rm = TRUE)
...
freq_provinces = SommaireQuali(base$province, na.rm = FALSE)
freq_revenu = SommaireQuanti(base$income, na.rm = TRUE)
...
```{r p_quali_quanti}
p_provinces_revenu = QualiQuanti(base$provinces, base$revenu)
...
freq_interet = SommaireQuali(base$p.intpol, na.rm = TRUE)
freq_revenu = SommaireQuanti(base$income, na.rm = TRUE)
...
La fonction QualiQuanti() du package personnalisé permet de calculer la valeur du p, la valeur du Tau de Kendall et de présenter un tableau qui montre la moyenne de la variable dépendante (Y) pour chaque groupe (valeurs possibles de la VI).
...
```{r p_T_quali_quanti}
p_Tau_polint_revenu = QualiQuanti(base$polint, base$revenu, Y_ord = TRUE)
...
freq_age = SommaireQuanti(base$age, na.rm = TRUE)
freq_revenu = SommaireQuanti(base$income, na.rm = TRUE)
...
La fonction QuantiQuanti() du package personnalisé permet de calculer la valeur du p, la valeur du r de Pearson et celle du R carré, en plus de l'analyse du modèle de régression linéaire. La fonction finie par présenter un graphique rudimentaire de la relation entre les deux variables qui inclue la droite de régression et son équation.
...
p_r_r2_age_revenu = QuantiQuanti(base$age, base$revenu)
